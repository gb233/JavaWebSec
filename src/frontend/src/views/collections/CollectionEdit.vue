<template>
  <div class="collection-edit">
    ...
    <el-card v-else-if="collection">
      <el-form :model="collectionForm" :rules="rules" ref="formRef" label-width="100px">
        ...
        <el-form-item label="标签">
          <el-input 
            ...
            @blur="parseTagsInput"
          />
          <div class="tags-preview" v-if="collectionForm.tags.length > 0">
            <el-tag 
        ...
        <el-form-item>
          ...
          <el-button @click="previewCollection" :loading="previewing">
            预览收藏夹
          </el-button>
    ...
    <!-- 预览对话框 -->
    <el-dialog v-model="showPreview" title="收藏夹预览" width="60%" top="5vh">
      <div class="collection-preview">
        <h2>{{ collectionForm.name }}</h2>
        <div class="collection-meta">
          <el-tag v-if="collectionForm.isPublic" type="success">公开</el-tag>
          <el-tag v-if="collectionForm.isDefault" type="warning">默认</el-tag>
        </div>
        <div class="collection-description" v-if="collectionForm.description">
          <h4>描述</h4>
          <p>{{ collectionForm.description }}</p>
        </div>
        <div class="collection-tags" v-if="collectionForm.tags.length > 0">
          <h4>标签</h4>
          <el-tag v-for="tag in collectionForm.tags" :key="tag" class="tag-item">{{ tag }}</el-tag>
        </div>
      </div>
    </el-dialog>
  </div>
