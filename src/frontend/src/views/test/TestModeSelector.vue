<template>
  <div class="test-mode-selector">
    ...
    <div class="mode-cards">
      <ElRow :gutter="24">
        <ElCol :span="8">
          <ElCard class="mode-card" @click="selectMode('realtime')">
        ...
        <ElCol :span="8">
          <ElCard class="mode-card" @click="selectMode('exam')">
            <div class="mode-content">
        ...
        <ElCol :span="8">
          <ElCard class="mode-card" @click="selectMode('random')">
            <div class="mode-content">
    ...
    <div v-if="recentRecords.length > 0" class="recent-records">
      ...
      <ElCard>
        <ElTable :data="recentRecords" style="width: 100%">
          <ElTableColumn prop="categoryName" label="测试分类" width="120" />
          <ElTableColumn prop="modeName" label="测试模式" width="120" />
          <ElTableColumn prop="score" label="得分" width="80" />
          <ElTableColumn prop="completionRate" label="完成率" width="100">
            <template #default="{ row }">
              <ElProgress :percentage="row.completionRate" :show-text="false" />
              <span style="margin-left: 8px">{{ row.completionRate }}%</span>
            </template>
          </ElTableColumn>
          <ElTableColumn prop="completedAt" label="完成时间" width="160" />
          <ElTableColumn label="操作" width="100">
            <template #default="{ row }">
              <ElButton type="primary" size="small" @click="viewResult(row.id)">
                查看详情
              </ElButton>
            </template>
          </ElTableColumn>
